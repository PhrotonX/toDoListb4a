...
Public Sub GetTasks(query As TaskQuery) As List
	Return m_database.TaskDao().GetTasks(query.GetJoinQuery(True), query.GetSearchingQuery(False), _
	"ORDER BY done ASC")
End Sub
...
Public Sub GetSortedTasks(query As TaskQuery) As List
	...
	If group_id == TASKS_DEFAULT Then
		Return m_database.TaskDao().GetTasks(query.GetJoinQuery(True), query.GetSearchingQuery(False), query.GetSortingQuery())
